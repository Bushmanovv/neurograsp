...
feature vector. Section~\ref{sec:fe_overview} provides an
...
Section~\ref{sec:fe_summary} summarises the chapter.
...
This design is motivated by the observation that the six
...
\textbf{Features} \\
\midrule
Frontal  & Fp1, Fp2            & Eye blinks (single, double, triple) & 38 \\
...
\begin{itemize}[leftmargin=*]
    ...
    \item Characteristic waveform shape (single, double, or
    triple peaks depending on blink count)
...
Frontal group (Fp1, Fp2) and are specifically designed to
discriminate between single, double, and triple blinks based
on the number and timing of deflection peaks within the
epoch window.
...
counted as peaks. $N_{\text{peaks}}$ directly encodes the
blink count: 1 for single blink, 2 for double blink, 3 for
triple blink.
...
The peak interval distinguishes double blinks (inter-blink
interval $\approx$200--400\,ms) from triple blinks
(shorter intervals, more peaks per unit time).
...
machine learning classifier with 2{,}821 training samples
(sample-to-feature ratio $\approx 30:1$, well above the
10:1 rule of thumb for stable classification), a feature
...
9  & Fp1 peak count                & Blink count (1/2/3) \\
10 & Fp2 peak count                & Blink count (1/2/3) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*]
    ...
    artifact family, not across all six classes).
    \item \textbf{Ranks 9--10:} Peak count at Fp1/Fp2
    directly encodes the blink count and is highly
    discriminative for single vs.\ double vs.\ triple
    blink classification.
\end{itemize}

\subsection{Feature Retention Decision}

All 93 features were retained in the final model (no
feature selection was applied at training time). This
decision was made because:

\begin{enumerate}[leftmargin=*]
    \item The sample-to-feature ratio (30:1) is adequate
    for Random Forest, which handles irrelevant features
    well through its internal random feature subsampling
    at each split.
...
extraction scheme producing a 93-dimensional feature vector
from each preprocessed \acrshort{eeg} epoch. Three channel
groups were defined based on physiological relevance: the
Frontal group (Fp1, Fp2; 38 features) captures blink
...
discriminative features. The following chapter describes
the classification and training methodology applied to
this feature vector.