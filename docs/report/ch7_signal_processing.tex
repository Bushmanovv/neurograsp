...
\node[box, below=of norm, fill=orange!20]  (feat)
    {$\rightarrow$ Feature Extraction (Chapter~8)};
...
\begin{itemize}[leftmargin=*]
    ...
    (two peaks separated by up to 600\,ms) with comfortable
    margin either side, while remaining short enough that the
    signal statistics are approximately stationary within each
    epoch. The same window length was retained from an earlier
    phase of the project in which a three-peak triple-blink
    class was still part of the taxonomy (up to 1{,}200\,ms
    span); triple blink was later dropped
    (Chapter~6), but the 2.0\,s window was kept since it also
    comfortably accommodates the sustained clinch and bruxism
    gestures.
...
Chapter~9. The overlap parameter was ultimately retained at
...
activity criterion, roughly half of the resulting epochs
correspond to rest windows — periods during which the subject
is sitting quietly between gestures (Chapter~6 reports 2{,}564
epochs before gating and 1{,}246 after, i.e.\ 51.4\% removed
overall, pooled across all three sessions and five classes).
These rest epochs contain
...
\begin{itemize}[leftmargin=*]
    ...
    \item A rest epoch from \code{BLINK01.edf} is labelled
    \code{single\_blink} but looks identical to the same
    background EEG.
    \item A rest epoch from a clinch recording is labelled
...
discarding epochs whose energy falls below a threshold. Rather
than using a single fixed threshold shared across every
recording, the gate implemented in \code{activity\_gate.py}
(function \code{gate\_mask}) is \textbf{recording-relative}:
the threshold is derived from each individual recording's own
activity distribution, so that differences in electrode
impedance, amplifier gain, or day-to-day signal scale between
sessions do not bias which epochs are kept.

The energy measure for epoch $k$ is computed as the maximum
\acrshort{rms} amplitude across the two physiologically most
informative channel groups:
...
For a given recording $r$ containing epochs $\{A_k\}_{k \in r}$,
the recording-specific threshold is:

\begin{equation}
\theta_r = \text{frac} \times P_{90}\!\left(\{A_k\}_{k \in r}\right)
\label{eq:gate_threshold}
\end{equation}

\noindent where $P_{90}(\cdot)$ denotes the 90th percentile of
the recording's own per-epoch activity distribution, and
\code{frac} $= 0.5$ is the gate fraction --- the same value used
consistently across \code{train\_hierarchical.py},
\code{train\_hybrid.py}, and \code{train\_dtw\_hybrid.py}
(Chapter~9). Epoch $k$ from recording $r$ is retained if and
only if:
...
Because $\theta_r$ is recomputed per recording, a quiet,
low-gain recording and a noisier, high-gain recording are each
gated relative to their own peak activity rather than against a
single global \acrshort{emg}/\acrshort{eog} amplitude assumed to
hold everywhere. This is also why the retention rate reported in
Chapter~6 is \emph{not} flat across classes (33.7\%--60.9\%):
retention now reflects each recording's own duration and
active/rest ratio, not a single shared cutoff.

\begin{lstlisting}[language=Python,
    caption={Recording-relative activity gate implementation
    (\code{activity\_gate.py}).},
    label={lst:gate}]
import numpy as np

def gate_mask(epochs, ch_names, frac=0.5):
    """
    Compute a boolean keep-mask for the epochs of a single
    recording, using a threshold relative to that recording's
    own 90th-percentile activity.

    epochs   : ndarray, shape (n_epochs, n_channels, n_samples)
    ch_names : list of channel name strings
    frac     : gate fraction (default 0.5)
    Returns  : boolean ndarray, shape (n_epochs,)
    """
    ...
    activity = np.array([
        max(np.sqrt(np.mean(e[fp_idx, :] ** 2)),
            np.sqrt(np.mean(e[t_idx,  :] ** 2)))
        for e in epochs
    ])

    threshold = frac * np.percentile(activity, 90)
    return activity >= threshold
\end{lstlisting}

\subsection{Design Rationale for the Gate Fraction}

The gate fraction $\text{frac} = 0.5$ was adopted, rather than a
single fixed microvolt threshold, once it became clear that a
shared global threshold does not transfer cleanly across
sessions recorded on different days with different electrode
impedances and amplifier gain (Chapter~6). Setting the threshold
to half of each recording's own 90th-percentile activity keeps
the epochs whose energy is clearly consistent with an active
gesture for \emph{that specific recording}, while discarding
epochs that sit near that recording's own resting baseline ---
without requiring the absolute microvolt scale to be comparable
between recordings. This is the gate fraction used consistently
throughout offline training and evaluation in Chapter~9. A
separate, fixed-microvolt activity gate is used at
\textbf{real-time inference} (Chapter~12), where no
whole-recording context is available to compute a percentile
on the fly; that gate is calibrated and reported separately in
that chapter and should not be conflated with the
recording-relative training-time gate described here.

\subsection{Impact of Activity Gating}

Applying \code{gate\_mask(frac=0.5)} across the full dataset
retains 1{,}246 of the 2{,}564 pre-gate epochs, an overall
retention of 48.6\% (Chapter~6, Table~\ref{tab:dataset_retention}).
Unlike a shared fixed threshold, this recording-relative gate
does \emph{not} retain a flat percentage per class: retention
ranges from 33.7\% (\code{clinch}) to 60.9\%
(\code{bruxism\_right}), reflecting differences in per-recording
duration and active/rest ratio across sessions rather than any
class-specific property of the background \acrshort{eeg}.
...
parameters (Chapter~8) — are extracted from the normalised
...
Activity gate fraction & 0.5        & Recording-relative (90th pct.) \\
...
based on \acrshort{rms} energy at Fp1/Fp2 and T3/T4, gated
against a \textbf{recording-relative} threshold set at 0.5
times each recording's own 90th-percentile activity level
(\code{gate\_mask(frac=0.5)}), rather than a single fixed
microvolt threshold shared across sessions. Per-recording
Z-score normalisation is applied to the entire signal before
epoching to ensure cross-session amplitude invariance. The
activity gate is the most significant innovation, retaining
1{,}246 of 2{,}564 epochs (48.6\%, Chapter~6) and yielding a
+3.1\% improvement in classification accuracy by eliminating
conflicting rest-window labels from the training set. A
separate, fixed-microvolt activity gate is used at real-time
inference (Chapter~12), where no whole-recording context is
available to compute a percentile on the fly. All offline
pipeline parameters are summarised in
Table~\ref{tab:pipeline_params}. The following chapter describes
the feature extraction scheme applied to the resulting clean,
activity-gated epochs.